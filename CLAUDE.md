...
- ✅ **Card-free trial (replaces strict card enforcement, July 2026)** — active trials need no card; expired trials hard-block via `needs_subscription`; signup is rate-limited (django-ratelimit, 5/h/IP, LocMemCache = per-worker).
...
- ✅ **Native Mobile App Phase 3** - Additional native iOS APIs for App Store compliance (Guideline 4.2)
  - Offline fallback page with NWPathMonitor network detection and auto-reconnect
  - Native share sheet (UIActivityViewController) on follow-ups, chat messages, and volunteer profiles
  - Universal Links via Apple App Site Association (AASA) endpoint at `/.well-known/apple-app-site-association`
  - Associated Domains entitlement (`applinks:aria.church`) for deep linking into chat, follow-ups, volunteers, comms, documents, interactions, analytics, care
  - Local notifications for follow-up reminders (UNUserNotificationCenter) — schedules at 9:00 AM on follow-up date
  - App shortcut quick actions (UIApplicationShortcutItem) — Chat with Aria, My Follow-ups, Log Interaction
...
│   ├── test_native_push.py   # Mobile app API & push notification tests (25 tests)
│   └── test_aasa.py          # Apple App Site Association endpoint tests (3 tests)
...
│   │   ├── app.js            # App init, tab navigation, More menu, deep link handler, network monitoring
...
### Native Mobile App Phase 3 — Additional Native iOS APIs (App Store Guideline 4.2)
Adds 5 more native iOS API integrations to strengthen the App Store case (12 total native APIs).

- **Offline Fallback Page** (`mobile/src/offline.html`, `mobile/src/app.js`):
  - NWPathMonitor-backed network detection via `@capacitor/network`
  - Custom offline page with gold-themed ARIA branding and retry button
  - Auto-reconnect: monitors `networkStatusChange` events, reloads when connection restored
  - `window.addEventListener('online')` fallback for WebView-level detection
- **Native Share Sheet** (`templates/base.html`, `templates/core/followup_detail.html`, `templates/core/chat_message.html`, `templates/core/volunteer_detail.html`):
  - `@capacitor/share` plugin triggers UIActivityViewController
  - Share buttons added to follow-up details, AI chat messages, and volunteer profiles
  - Guarded by `{% if is_app_mode %}` — only visible in native app
  - Global script in `base.html` finds `data-share-title` elements and wires up share functionality
- **Universal Links / Deep Linking** (`config/urls.py`, `mobile/ios/App/App/App.entitlements`, `mobile/src/app.js`):
  - Apple App Site Association (AASA) served at `/.well-known/apple-app-site-association`
  - App ID: `454YNT65J3.church.aria.app`
  - Supported paths: `/chat/*`, `/followups/*`, `/volunteers/*`, `/comms/*`, `/documents/*`, `/interactions/*`, `/analytics/*`, `/care/*`
  - Associated Domains entitlement: `applinks:aria.church`
  - `@capacitor/app` plugin listens for `appUrlOpen` events and navigates WebView
  - 3 tests in `tests/test_aasa.py` (content type, app ID, paths)
- **Local Notifications for Follow-up Reminders** (`templates/base.html`, `templates/core/followup_detail.html`, `templates/core/partials/followup_row.html`):
  - `@capacitor/local-notifications` schedules reminders via UNUserNotificationCenter
  - Data attributes (`data-followup-id`, `data-followup-title`, `data-followup-date`, `data-followup-status`) on follow-up elements
  - Schedules notification at 9:00 AM on follow-up date
  - Auto-cancels for completed/cancelled follow-ups
  - Notification tap navigates to follow-up detail page
  - Bulk scheduling from follow-up list page via `followup_row.html` data attributes
- **App Shortcut Quick Actions** (`mobile/ios/App/App/Info.plist`, `mobile/ios/App/App/AppDelegate.swift`):
  - 3 UIApplicationShortcutItems in Info.plist: Chat with Aria, My Follow-ups, Log Interaction
  - `performActionFor shortcutItem` handler in AppDelegate maps shortcut types to URLs
  - Cold-start handling: `pendingShortcutURL` stored, injected into WebView via `evaluateJavaScript` after 3s delay in `applicationDidBecomeActive`
- **Mobile npm Dependencies Added** (`mobile/package.json`):
  - `@capacitor/network` — Network status monitoring
  - `@capacitor/share` — Native share sheet
  - `@capacitor/app` — Deep link / universal link handling
  - `@capacitor/local-notifications` — Local notification scheduling
- **12 Total Capacitor Plugins** (confirmed via `npx cap sync ios`):
  - `@capacitor/app`, `@capacitor/browser`, `@capacitor/haptics`, `@capacitor/local-notifications`, `@capacitor/network`, `@capacitor/preferences`, `@capacitor/push-notifications`, `@capacitor/share`, `@capacitor/splash-screen`, `@capacitor/status-bar`, `@capgo/capacitor-native-biometric`, `@nicknisi/capacitor-native-badge`
- **3 new tests** in `tests/test_aasa.py`:
  - AASA returns JSON with correct content type
  - Response contains applinks with correct app ID (454YNT65J3)
  - Specifies expected URL path components
  - Full suite: **452 tests passing, 0 failures, 0 errors**
