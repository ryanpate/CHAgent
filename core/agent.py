...
def is_song_or_setlist_query(message: str) -> Tuple[bool, str, Optional[str]]:
    ...
    if is_song_query:
        ...
        if query_type in ['setlist', 'team_schedule'] and not extracted_value:
            date_patterns = [
                r'(last\s+sunday|this\s+(?:coming\s+)?sunday|next\s+sunday|coming\s+sunday|yesterday|today|tomorrow|this\s+weekend|next\s+weekend|last\s+weekend|(?:this\s+)?(?:coming\s+)?weekend)',
